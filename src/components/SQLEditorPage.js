...
function SQLEditorPage() {
  ...
  const handleConnectToDb = async () => {
    ...
    try {
      const response = await fetch(API_ENDPOINTS.CONNECT_DB, {
        method: "POST",
  ...
  const handleConvertToSql = async () => {
    ...
    try {
      ...
      if (useDbSchema && isConnected) {
        try {
          const schemaResponse = await fetch(API_ENDPOINTS.GET_FULL_SCHEMA);
      ...
      const response = await fetch(API_ENDPOINTS.GENERATE_QUERY, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
