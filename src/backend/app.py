...
@app.route("/generate-data-from-sql", methods=["POST"])
def generate_data_from_sql():
    data = request.json

    create_sql = data.get("createTableSQL")
    count = data.get("count", 10)

    if not create_sql:
        return jsonify({"error": "CREATE TABLE SQL required"}), 400

    try:
        schema_json = create_table_sql_to_json(create_sql)
        schema_json["count"] = count
...
def create_table_sql_to_json(sql):
    sql = sql.strip().rstrip(";")

    # Extract table name
    table_match = re.search(r"CREATE\s+TABLE\s+(\w+)", sql, re.IGNORECASE)
    if not table_match:
        raise Exception("Invalid CREATE TABLE statement")

    table_name = table_match.group(1)

    # Extract column definitions
    columns_block = re.search(r"\((.*)\)", sql, re.DOTALL)
    if not columns_block:
        raise Exception("Invalid column definition")

    column_lines = columns_block.group(1).split(",")
    ...
    for line in column_lines:
        ...
        if "int" in col_type_raw:
            col_type = "int"
        elif "char" in col_type_raw or "text" in col_type_raw:
            col_type = "string"
